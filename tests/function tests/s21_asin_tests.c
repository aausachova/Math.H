...
START_TEST(s21_asin_1) {
  ...
  ck_assert_ldouble_eq_tol(s21_asin(test_val), asin(test_val), 1e-06);
}
END_TEST
...
START_TEST(s21_asin_21) {
  for (int i = 0; i < 5; ++i) {
    ...
    ck_assert_ldouble_eq_tol(s21_asin(test_val_m), asin(test_val_m), 1e-06);
  }
}
END_TEST
...
Suite* s21_asin_suite_create(void) {
  Suite* suite = suite_create(
      "\033[1;35m||========================= \033[0m тест s21_asin \033[1;35m "
      "=======================||\033[0m\n\n");
  ;
